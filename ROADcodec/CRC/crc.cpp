...
const ROADUInt16 Crc16Table[256] = {
...
ROADUInt16 CRCSupport::CRC::CRC16(ROADPtrByte aData, ROADUInt32 aLength)
{
    ...
    while (aLength--)
        lresult = (lresult >> 8) ^ Crc16Table[(lresult & 0xFF) ^ *aData++];
...
ROADUInt32 CRCSupport::CRC::CRC32(ROADPtrByte aData, ROADUInt32 aLength)
{
    ...
    ROADUInt32 crc_table[256];

    for (i = 0; i < 256; i++)
    {
        lresult = i;
        for (j = 0; j < 8; j++)
            lresult = lresult & 1 ? (lresult >> 1) ^ 0xEDB88320UL : lresult >> 1;
